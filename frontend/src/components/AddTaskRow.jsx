...
export default function AddTaskRow({ onAdd }) {
  ...
  return (
    <div style={{ padding: '0 16px', height: 44, display: 'flex', alignItems: 'center', gap: 12 }}>
      <div
        style={{
          width: 22, height: 22, borderRadius: '50%',
          border: '1.5px dashed #C7C7CC', flexShrink: 0,
          display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer',
        }}
        onClick={() => { setFocused(true); setTimeout(() => inputRef.current?.focus(), 50); }}
      >
        <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
          <path d="M5 2v6M2 5h6" stroke="#C7C7CC" strokeWidth="1.5" strokeLinecap="round" />
        </svg>
      </div>
      {focused ? (
        <input
        ...
        <span
          style={{ fontSize: 15, color: '#C7C7CC', cursor: 'text' }}
          onClick={() => { setFocused(true); setTimeout(() => inputRef.current?.focus(), 50); }}
        >
          새 태스크 추가...
        </span>
